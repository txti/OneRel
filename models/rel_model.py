...
class RelModel(nn.Module):
    def __init__(self, config):
        ...
        self.bert_encoder = BertModel.from_pretrained(
            "bert-base-cased", cache_dir="./pre_trained_bert"
        )
    ...
    def triple_score_matrix(self, encoded_text, train=True):
        ...
        head_representation = (
            encoded_text.unsqueeze(2)
            .expand(batch_size, seq_len, seq_len, bert_dim)
            .reshape(batch_size, seq_len * seq_len, bert_dim)
        )
        ...
        triple_scores = self.relation_matrix(entity_pairs).reshape(
